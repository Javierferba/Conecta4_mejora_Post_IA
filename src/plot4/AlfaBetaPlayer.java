 ...
 * @version 1.2
 *
 * @brief Este es el algoritmo PodaAlfaBeta para un tablero n*m para jugar al juego conecta-k donde k, n y m vienen definidas
 * en la clase Grid. Esta clase hereda el método turno de la clase Player, el cual, devolverá la columna en la que el
 * algoritmo AlfaBeta habrá decidido colocar su ficha. Este algoritmo cuenta con un árbol (podado) y una serie de
...
public class AlfaBetaPlayer extends Player {
    ...
    private  byte profundidadMax = 50;
    ...
    private int alfa = Integer.MAX_VALUE; /// IA.
    private int beta = Integer.MIN_VALUE; /// Jugador.

    private boolean completo = false;/// Verdadero cuando ya no se pueda ampliar más la raíz.
    ...
    private void rellena(Nodo nodo) {
        ...
        if (nodo.profundidad%2==0){
            if (beta<nodo.valor){
                beta = nodo.valor;
            } else if (beta>nodo.valor) {
                return;
            }
        } else {
            if (alfa>nodo.valor){
                alfa = nodo.valor;
            } else if (alfa<nodo.valor) {
                return;
            }
        }
     ...
     * @brief mueve el parametro raiz a uno de sus hijos actualizando así el caso actual y ampia el arbol.
     */
    public void actualizar(byte[][] m) {
        byte profRaizAnterior = _raiz.profundidad;
        _raiz = buscaEstado(m);
        if(!completo) {
            profundidadMax += (byte) (_raiz.profundidad - profRaizAnterior);
            amplia();
        }
    }
    ...
    public byte recorridoInorden(Nodo nodo) {
        ...
        if (nodo._hijos.isEmpty()) {
            ...
            for (int i = 0; i < nodo._hijos.size(); i++) {
                ...
                if (nodo.profundidad%2==0 && candidato==1) {
                    contador++;
                } else if (nodo.profundidad%2!=0 && candidato==-1){
                    contador--;
                } else if (candidato==0 && posiblesHuecos(nodo._estado)==null) {
                    /**
                     * @todo comprobar si es hoja y en caso de o serlo evaluar el nodo.
                     */
                }
            }
            ...
            for (int i = 1; i < nodo._hijos.size(); i++) {
    ...
    /**
     * @brief Amplía el árbol (llamada desde la raíz). Busca los nodos hoja y si no son casos finales amplia el
     * árbol hasta llegar a la nueva profundidad máxima.
     */
    private void amplia() {
        for (byte i = 0; i < _columnas; i++) {
            ampliaRecursivo(_raiz._hijos.get(i));
        }
    }

    /**
     * @brief Ampliación recursiva.
     * @param nodo nodo de búsqueda o ampliación.
     */
    private void ampliaRecursivo(Nodo nodo) {
        if (nodo._hijos.size()!=0) {
            for (byte i = 0; i < _columnas; i++) {
                ampliaRecursivo(nodo._hijos.get(i));
            }
        } else if (nodo.valor==0 && posiblesHuecos(nodo._estado)!=null){
            rellena(nodo);
        }
    }
